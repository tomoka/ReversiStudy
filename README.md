...
| 段階 | 読みの深さ | 終盤の読み切り | わざと外す割合 |
| --- | --- | --- | --- |
| 1 入門 | 1 手 | なし | 70% |
| 2 初級 | 2 手 | 残り 4 マス | 30% |
| 3 中級（既定） | 4 手 | 残り 8 マス | 0% |
| 4 上級 | 6 手 | 残り 10 マス | 0% |
| 5 達人 | 7 手 | 残り 12 マス | 0% |

浅い読みでも隅を優先する評価は効いてしまうため、弱い段階ではときどき
わざと最善から外す。設定はすべて `Difficulty` にある。

強い段階は 1 手に 1 秒以上かかることがあるので、探索は UI スレッドから
外して別スレッドで行う。局面が変わったら、走っている探索の結果は捨てる。
